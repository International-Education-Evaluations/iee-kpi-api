  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-display font-bold text-white">Email Reports</h1>
          <p className="text-xs text-slate-400 mt-0.5">{schedules.length} configured · SendGrid dynamic templates</p></div>
        {isManagerPlus() && <button onClick={() => setShowCreate(!showCreate)} className="px-4 py-2 bg-navy-600 hover:bg-navy-500 text-white text-sm rounded-lg font-medium">+ New Schedule</button>}
      </div>

      {lastResult && <div className="glass rounded-xl p-4 border-emerald-500/20 bg-emerald-600/5">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-emerald-400">Email sent successfully</span>
          <button onClick={() => setLastResult(null)} className="text-[10px] text-slate-400 hover:text-white">Dismiss</button>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div><span className="text-slate-400">Date:</span> <span className="text-white">{lastResult.report_date}</span></div>
          <div><span className="text-slate-400">Active:</span> <span className="text-white">{fmtI(lastResult.total_active_orders)}</span></div>
          <div><span className="text-slate-400">Waiting:</span> <span className="text-white">{fmtI(lastResult.waiting_orders)}</span></div>
          <div><span className="text-slate-400">&gt;72h:</span> <span className="text-red-400 font-medium">{fmtI(lastResult.orders_over_72h)}</span></div>
        </div>
      </div>}
      ...
      {showCreate && <div className="glass rounded-xl p-5">
        ...
        <form onSubmit={create} className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
          <div><label className="text-[10px] text-slate-400 uppercase">Schedule Name</label>
          ...
          <div><label className="text-[10px] text-slate-400 uppercase">Recipients (comma-separated)</label>
            <input type="text" value={form.recipients} onChange={e => setForm(f => ({...f, recipients: e.target.value}))} required placeholder="ops@myiee.org, lead@myiee.org"
              className="w-full mt-1 px-3 py-1.5 bg-slate-800/60 border border-slate-600/40 rounded text-sm text-white" /></div>
          <div><label className="text-[10px] text-slate-400 uppercase">Frequency</label>
          ...
          <div><label className="text-[10px] text-slate-400 uppercase">Report Type</label>
          ...
          <div className="col-span-full flex gap-2">
            <button type="submit" disabled={saving} className="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs rounded font-medium">{saving ? 'Creating...' : 'Create'}</button>
            <button type="button" onClick={() => setShowCreate(false)} className="px-4 py-1.5 bg-slate-700 text-white text-xs rounded">Cancel</button>
          </div>
        </form>
      </div>}
        ...
        <table className="tbl w-full">
          ...
          <tbody>{schedules.map(s => (
            <tr key={s._id}>
              ...
              <td className="text-xs text-slate-400 max-w-[200px] truncate">{(s.recipients || []).join(', ')}</td>
              <td className="text-xs">{s.frequency}</td>
              <td className="text-xs text-slate-400">{s.reportType}</td>
              <td>{s.enabled ? <span className="text-emerald-400 text-xs">Active</span> : <span className="text-slate-500 text-xs">Paused</span>}</td>
              <td className="text-xs text-slate-500">{s.lastSentAt ? new Date(s.lastSentAt).toLocaleString('en-US', { timeZone: 'America/New_York' }) : 'Never'}</td>
              <td className="text-right">
                <div className="flex gap-1 justify-end">
                  <button onClick={() => sendNow(s)} disabled={sending === s._id}
                    className="text-[10px] text-navy-400 hover:text-white px-2 py-0.5 border border-slate-700/40 rounded">
                    {sending === s._id ? 'Sending...' : 'Send Now'}
                  </button>
                  <button onClick={() => toggle(s)} className={`text-[10px] px-2 py-0.5 border rounded ${s.enabled ? 'text-amber-400 border-amber-500/20' : 'text-emerald-400 border-emerald-500/20'}`}>
                    {s.enabled ? 'Pause' : 'Enable'}
                  </button>
                  <button onClick={() => remove(s)} className="text-[10px] text-red-400 border border-red-500/20 rounded px-2 py-0.5">Delete</button>
                </div>
              </td>
            </tr>
          ))}</tbody>
        </table>
        {!schedules.length && <div className="p-8 text-center text-slate-500 text-sm">No email schedules configured. Click + New Schedule to set up automated reports.</div>}
      </div>
          ...
          <table className="tbl w-full"><thead><tr><th>Variable</th><th>Type</th><th>Description</th></tr></thead>
            <tbody>
              {[
                ...
                <tr key={i}><td className="font-mono text-xs text-emerald-400">{'{{' + v + '}}'}</td><td className="text-xs text-slate-400">{t}</td><td className="text-xs">{d}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
