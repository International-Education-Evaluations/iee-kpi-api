  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-display font-bold text-ink-900">AI Configuration</h1>
          <p className="text-xs text-ink-400 mt-0.5">Control chatbot behavior, guardrails, and system prompt</p></div>
        {msg && <span className="text-emerald-600 text-xs bg-emerald-50 border border-emerald-200 px-3 py-1 rounded">{msg}</span>}
      </div>
      ...
      <div className="card-surface p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
          <div>
            <label className="text-[10px] text-ink-400 uppercase block mb-1">Max Days (query window)</label>
            ...
            <p className="text-[10px] text-ink-500 mt-0.5">Hard cap: 365</p>
          </div>
          <div>
            <label className="text-[10px] text-ink-400 uppercase block mb-1">Max Page Size</label>
            <input type="number" value={guardrails.maxPageSize || 2000} onChange={e => upd('maxPageSize', parseInt(e.target.value) || 2000)}
              className="w-full px-3 py-1.5 bg-white border border-surface-200 rounded text-sm text-ink-900" />
            <p className="text-[10px] text-ink-500 mt-0.5">Hard cap: 5000</p>
          </div>
          <div>
            <label className="text-[10px] text-ink-400 uppercase block mb-1">Max Tool Iterations</label>
            <input type="number" value={guardrails.maxToolIterations || 5} onChange={e => upd('maxToolIterations', parseInt(e.target.value) || 5)}
              className="w-full px-3 py-1.5 bg-white border border-surface-200 rounded text-sm text-ink-900" />
            <p className="text-[10px] text-ink-500 mt-0.5">Hard cap: 10</p>
          </div>
          <div>
            <label className="text-[10px] text-ink-400 uppercase block mb-1">Max Tokens (response length)</label>
            <input type="number" value={guardrails.maxTokens || 4096} onChange={e => upd('maxTokens', parseInt(e.target.value) || 4096)}
              className="w-full px-3 py-1.5 bg-white border border-surface-200 rounded text-sm text-ink-900" />
            <p className="text-[10px] text-ink-500 mt-0.5">Hard cap: 8192</p>
          </div>
          <div>
            <label className="text-[10px] text-ink-400 uppercase block mb-1">Claude Model</label>
          ...
          <div>
            <label className="text-[10px] text-ink-400 uppercase block mb-1">Rate Limit (per min)</label>
          ...
          <div className="flex items-center gap-3 col-span-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={guardrails.summaryOnly !== false} onChange={e => upd('summaryOnly', e.target.checked)}
                className="w-4 h-4 rounded border-surface-200 bg-white text-navy-500" />
              <span className="text-sm text-ink-600">Summary only (no raw rows)</span>
            </label>
          </div>
        </div>

        <div className="mt-4">
          <label className="text-[10px] text-ink-400 uppercase block mb-2">Allowed Tools</label>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {ALL_TOOLS.map(t => (
              <label key={t.id} className={`flex items-start gap-2.5 p-2.5 rounded-lg border cursor-pointer transition-colors ${
                (guardrails.allowedTools || []).includes(t.id) ? 'border-emerald-500/30 bg-emerald-600/5' : 'border-surface-200 bg-white/20 opacity-50'
              }`}>
                <input type="checkbox" checked={(guardrails.allowedTools || []).includes(t.id)} onChange={() => toggleTool(t.id)}
                  className="w-4 h-4 mt-0.5 rounded border-surface-200 bg-white text-emerald-500 shrink-0" />
                <div>
        ...
        <button onClick={saveGuardrails} disabled={saving}
          className="mt-4 px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-surface-200 disabled:text-ink-400 text-ink-900 text-sm rounded-lg font-medium">
      ...
      <div className="card-surface p-5">
        ...
        <textarea value={prompt} onChange={e => setPrompt(e.target.value)} rows={14}
          className="w-full mt-3 px-4 py-3 bg-white border border-surface-200 rounded-lg text-sm text-ink-900 font-mono resize-y focus:outline-none focus:border-brand-400 leading-relaxed" />
        <div className="flex items-center gap-3 mt-3">
          <button onClick={savePrompt} disabled={savingPrompt}
            className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-surface-200 disabled:text-ink-400 text-ink-900 text-sm rounded-lg font-medium">
            {savingPrompt ? 'Saving...' : 'Save Prompt'}
          </button>
          <span className="text-[10px] text-ink-500">Changes affect all users immediately. All edits are logged in the audit trail.</span>
        </div>
      </div>
    </div>
  );
